...
class FeatureExtractorBackbone(Backbone):
    ...
    def __init__(
        ...
        min_stride: int = 4,
        max_stride: int = 32,
        projection_dim: List[int] = [128, 256, 512, 1024],
        ...
        for indices in self._sorted_grouped_indices:
            ...
            self._out_feature_channels[name] = projection_dim[idx]
            for index in indices:
                self.feature_projections[str(index)] = nn.Sequential(
                    *ResNet.make_stage(
                        BottleneckBlock,
                        num_blocks=num_res_blocks,
                        in_channels=self.feature_extractor.feature_dims[index],
                        bottleneck_channels=projection_dim[idx] // 4,
                        out_channels=projection_dim[idx],
                        norm="GN",
                    )
                )
    ...
    def forward_features(self, features, input_image_size):
        ...
        for name, indices in zip(self._out_features, self._sorted_grouped_indices):
            ...
            for idx in indices:
                ...
                projected_feature = self.feature_projections[str(idx)](restored_feature)
