...
class MaskCLIP(ClipAdapter):
    ...
    def pred_logits(self, mask_embed, text_embed, 
                    # labels
                    ):
        logit_per_mask = (
            torch.einsum(
                # "bqc,nc->bqn", F.normalize(mask_embed, dim=-1), F.normalize(text_embed, dim=-1)
                "bqc,bc->bq", F.normalize(mask_embed, dim=-1), F.normalize(text_embed, dim=-1)
            )
            * self.logit_scale
        )
    ...
    def forward(self, image, mask, text_embed, 
                # labels
                ):
        ...
        if text_embed is not None:
            ...
            output["mask_pred_open_logits"] = self.pred_logits(mask_embed, text_embed)
